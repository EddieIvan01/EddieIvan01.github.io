...
summary: CUMTCTF 双月赛I && CUMTCTF 双月赛II && 2019安恒一月月赛 && Teaser CONFidence CTF && DDCTF2019 && *CTF2019
...
太菜了，Web就做出一道题，剩下两道一道页游一脸懵逼（虽然提示了跟Mongo有关），另一道tm是不是给错category了啊（Pwn爷爷rbq）。Misc里的`homebrew eventloop`很有意思，和DDCTF2019里的是同款，garzon大佬出的，研究了一会儿想着怎么绕过`sys.stdin`的赋值就可以通过报错getflag，之后提示所有bug都是有意的，又思考那个`split(xx, '114514')`确定是有意的吗，我觉得是作者fp写多了以为Python里列表解包能`[head | tail]`。之后....之后就去复习OS了....2019*CTF之旅end
