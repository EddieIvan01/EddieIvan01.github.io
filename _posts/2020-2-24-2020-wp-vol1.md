...
可设置$_SESSION['token']为admin，直接登录后即可拿flag
...
PDO堆叠 + 宽字节 + 延时，脚本跑了半天。延时注入有必要弄一堆奇怪又长的列名来浪费时间吗，而且题目本身就select了一列，flag占一列，为什么要设置总共>=四列，觉得你的列名很萌萌哒吗
...
Python3.7.4的SSTI，down了个Python3.7的解释器，发现内置类的架构被重构了，之前常用的payload已经不能通过`object.__subclasses__`访问到了
...
+ 将query拼接到`/source?`后面，丢给http.get请求，此时会编码一次
+ CRLF注入一个POST请求访问file_upload接口，HTTP协议报文的控制字符需要是raw的

因为payload是在最外层报文的GET query里，所以只能利用node.js转义unicode的feature来处理空格，引号等

注入一个POST请求成功后，需要上传一个pug文件，google一下pug模板即可。字符串过滤随便绕一下就行了
...
上传后需要加载模板，可以看到`/?action=`可加载，不过限制了路径穿越，这里就很刻意了，上传路由里保存了两次，另一个副本保存在`uploads/MIME_TYPE/FEIL_NAME`，在MIME里跳一下目录就行了
